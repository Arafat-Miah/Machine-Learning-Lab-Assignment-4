...
step = @(t) double(t >= 0);            % IMPLEMENT THE ANONONYMOUS FUNCTION

% Define the step model function using the step function
model_step = @(x,w) step( [ones(size(x,1),1) x] * (w.' ) );    % IMPLEMENT THE ANONONYMOUS FUNCTION

% Define the step cost function using the model_step
cost_step_LS = @(w) (1/P) * sum( ( model_step(x,w) - y ).^2, 1 ).';    % IMPLEMENT THE ANONONYMOUS FUNCTION

% Define the sigmoid function
sigmoid = @(x) 1 ./ (1 + exp(-x));         % IMPLEMENT THE ANONONYMOUS FUNCTION

% Define the Logistic regression model using sigmoid function
model_logit = @(x,w) sigmoid( [ones(size(x,1),1) x] * (w.' ) );   % IMPLEMENT THE ANONONYMOUS FUNCTION

% Define the Logistic regression Least Squares cost function
cost_logit_LS = @(w) (1/P) * sum( ( model_logit(x,w) - y ).^2, 1 ).';   % IMPLEMENT THE ANONONYMOUS FUNCTION

% Define the Logistic regression Cross-Entropy cost function
cost_logit_CE = @(w) (1/P) * sum( ...
    -( y .* log( model_logit(x, w) ) + (1 - y) .* log( 1 - model_logit(x, w) ) ), ...
    1 ).';   % IMPLEMENT THE ANONONYMOUS FUNCTION
